...
//   const convertLeadId = location.state?.convertLeadId || "";
//   // =================================================
//   // STATES
//   // =================================================
...
//       convertLeadId !== ""
//     ) {
//       console.log("Lead converted to Deal. Lead ID:", convertLeadId);
...
//     if (location.state?.convertLeadId) {
//       navigate("/dealslist", {
//         replace: true,
//         state: {},
//       });
...
//     // CLOSE DATE SEARCH
//     // -------------------------------------------------
//     const closeDateValue = deal.close_date ? dayjs(deal.close_date) : null;
...
//           closeDateValue.format("MM/DD/YYYY"),
//           closeDateValue.format("DD MMM YYYY").toLowerCase(),
//           closeDateValue.format("MMM DD, YYYY").toLowerCase(),
//         ]
//       : [];

//     const matchesCloseDateSearch = closeDateFormats.some((date) =>
...
//     const matchesOwner =
//       !dealOwner ||
//       (Array.isArray(deal.deal_owners) && deal.deal_owners.includes(dealOwner));
//     // -------------------------------------------------
//     // CLOSE DATE FILTER
//     // -------------------------------------------------
...
//       !closeDate ||
//       (deal.close_date &&
//         dayjs(deal.close_date).format("YYYY-MM-DD") === closeDate);
//     // -------------------------------------------------
//     // CREATED DATE FILTER
//     // -------------------------------------------------
...
//       !createdDate ||
//       (deal.created_date &&
//         dayjs(deal.created_date).format("YYYY-MM-DD") === createdDate);
//     return (
//       matchesSearch &&
//       matchesStage &&
//       matchesOwner &&
...
//         Array.isArray(deal.deal_owners) ? deal.deal_owners : [],
//       ),
//     ),
//   ];
//   // =================================================
...
//             onChange={(newValue) =>
//               setCloseDate(newValue ? newValue.format("YYYY-MM-DD") : "")
//             }
//           />
//           {/* CREATED DATE */}
//           <CommonDatePicker
...
//                       fontWeight: 500,
//                     }}
//                     onClick={() => navigate(`/deals/${deal.id}/activities`)}
//                   >
//                     {deal.deal_name}
//                   </Box>
//                 </TableCell>
...
function DealsList() {
  ...
  const fetchDealStages = async () => {
    try {
      ...
      showToast("Failed to load deal stages.", "error");
    }
  };
  ...
  const filteredDeals = dealsData.filter((deal) => {
    ...
    const matchesCloseDateSearch = closeDateFormats.some(
      (date) => date.includes(searchText),
    );
    ...
    // -------------------------------------------------
    const matchesStage =
      !dealStage || deal.deal_stage === dealStage;
    ...
    const matchesOwner =
      !dealOwner ||
      (Array.isArray(deal.deal_owners) &&
        deal.deal_owners.includes(dealOwner));
    ...
    const matchesCloseDate =
      !closeDate ||
      (deal.close_date &&
        dayjs(deal.close_date).format("YYYY-MM-DD") ===
          closeDate);
    ...
    const matchesCreatedDate =
      !createdDate ||
      (deal.created_date &&
        dayjs(deal.created_date).format("YYYY-MM-DD") ===
          createdDate);
    ...
    >
      ...
      <FilterSection>
        ...
        <CommonDatePicker
          label="Created Date"
          value={
            createdDate ? dayjs(createdDate) : null
          }
          onChange={(newValue) =>
            setCreatedDate(
              newValue
